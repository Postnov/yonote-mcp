...
- [x] Phase 2 решает проблему неполных text-полей: ProseMirror-списки/таблицы отсутствуют в `text`, но есть в export
- [x] 255 тестов, все проходят

## Где остановились
**Дата:** 2026-02-17

`deep_search` доработан двухфазным алгоритмом: Phase 1 быстро сканирует text-поля, Phase 2 (при 0 результатах) экспортирует каждую страницу через `document_export_markdown` для получения полного контента, включая ProseMirror-блоки (списки, таблицы). Это решает проблему, когда "керамогранит" находится в списке и не попадает в поле `text`.
