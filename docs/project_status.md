...
Реализовано обогащение `extract_sections` и `copy_section` картинками через `attachments.list`. Картинки привязаны к документу (не к секции), поэтому добавляются в конец извлечённой секции.
